...
if 'E' in data_resources[lang_train]:
    # existing train data
    f_existing_train = open('/nlsasfs/home/ai4bharat/manidl/yash/indic-xlit/all_data/existing_data/Indic_Xlit_translit_parallel_corpus/'+lang_train+'/'+lang_abr_train+'_train.txt','r')
...
if 'M' in data_resources[lang_train]:
    # Manual transliteration pairs train data
    f_manual_train = open('../benchmark_data_9_april/train_set/'+lang_abr_train+'_train.txt','r')
    lines_manual_train = f_manual_train.read().split('\n')
    lines_train += lines_manual_train
    print("manual train pairs: ",len(lines_manual_train))
...
lang_abr_test_valid_list = ['as', 'bn', 'brx', 'gom', 'gu', 'hi', 'kn', 'ks', 'mai', 'ml', 'mni', 'mr', 'ne', 'or', 'pa', 'sa', 'ta', 'te', 'ur']
lang_test_valid_list = ['Assamese', 'Bangla', 'Bodo', 'Konkani', 'Gujarati', 'Hindi', 'Kannada', 'Kashmiri', 'Maithili', 'Malayalam', 'Manipuri', 'Marathi', 'Nepali', 'Oriya', 'Punjabi', 'Sanskrit', 'Tamil', 'Telugu', 'Urdu']
...
for lang_test_valid, lang_abr_test_valid in zip(lang_test_valid_list, lang_abr_test_valid_list):
    ...
    # named entities
    f_in_name = open('../benchmark_data_9_april/Named_entities/normalized_names_files/'+lang_abr_test_valid+'_benchmark_names.txt','r')
    lines_name = f_in_name.read().split('\n')

    lines_name = [line for line in lines_name if line]
    print("Benchmark named entities: ",len(lines_name))

    lines_name = [line for line in lines_name if len( line.split('\t') ) == 2 ]
    print("Benchmark named entities after preprocessing : ",len(lines_name))

    lines_name = ['\t'.join(line.split('\t')[:2]) for line in lines_name]
    print("Benchmark named entities : ",len(lines_name))


    pattern = '[^a-zA-Z]'    
    lines_name = [line for line in lines_name if not re.compile(pattern).search(line.split('\t')[1]) ]
    print("total named entities (removing words containing non-english characters) : ",len(lines_name))

    lines_name = [line for line in lines_name if not re.compile(lang_patterns_test_valid_dict[lang_test_valid]).search(line.split('\t')[0]) ]
    print("total named entities (removing words containing non-hindi characters) : ",len(lines_name))

    all_lang_lines_test_valid += lines_name
    f_in_name.close()
    ...
    lines_valid = [line for line in lines_valid if line]
    print("Benchmark valid pairs: ",len(lines_valid))
...
for lang_test_valid, lang_abr_test_valid in zip(lang_test_valid_list, lang_abr_test_valid_list):
    ...
    # valid data: random sample from mined data 
    f_in_random_samp = open('../mined_data/valid_set_random_sample/valid_random_sample_2k_'+lang_abr_test_valid+'.txt','r')
    lines_random_samp = f_in_random_samp.read().split('\n')
    ...
    all_lang_lines_test_valid += lines_random_samp
...
for lang_test_valid, lang_abr_test_valid in zip(lang_test_valid_list, lang_abr_test_valid_list):
    ...
    # test data
    f_in_test = open('../benchmark_data_9_april/benchmark_native_words/native_words_'+lang_abr_test_valid+'.txt','r')
    lines_test = f_in_test.read().split('\n')
